...
const view = ref<'diary' | 'ledger'>('diary')
...
const datesWithEntries = computed(() => view.value === 'diary' ? diaryDates.value : new Set(ledgerDates.value))
...
// 日记和账本共用日期；切换模块前先提交日记的待保存内容。
async function changeView(next: 'diary' | 'ledger') {
  if (busy.value || !ready.value || next === view.value) return
  busy.value = true
  try { if (await ledger.value?.beforeClose() === false) return; await flush(); view.value = next }
  catch (e) { report(e) } finally { busy.value = false }
}
async function backup(action: 'export' | 'import') {
  ...
  try {
    ...
    if (action === 'export') { if (await window.journal.exportBackup()) notice.value = '备份已导出，请妥善保存。' }
      ...
      if (count !== null) { await refresh(); await load(selected.value); refreshToken.value++; notice.value = `已恢复 ${count.diaries} 篇日记、${count.transactions} 笔账目。` }
    }
  } catch (e) { report(e) } finally { busy.value = false }
}
...
function shortcut(event: KeyboardEvent) {
  if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 's') { event.preventDefault(); if (view.value === 'ledger') void ledger.value?.saveDraft(); else void saveNow() }
...
onMounted(async () => {
  ...
  unsubscribe = window.journal?.onCloseRequest(async () => {
    ...
    busy.value = true
    try {
      if (await ledger.value?.beforeClose() === false) { busy.value = false; return }
  ...
  <div class="shell">
    <aside class="sidebar">
      ...
      <section class="calendar" aria-label="日历">
        ...
        <div class="calendar-foot"><span>{{ view === 'diary' ? `本月记录了 ${monthCount} 天` : `本月 ${ledgerDates.length} 笔账目` }}</span><button :disabled="busy || !ready" @click="select(today)">回到今天 ↗</button></div>
      </section>
      ...
      <div class="sidebar-bottom"><span class="local-dot"></span> 本地手账 · 无需联网 <small>v0.2</small></div>
    </aside>
    <main>
      <header class="topbar"><span>我的手账 <span class="slash">/</span> {{ view === 'diary' ? '日记' : '生活账本' }}</span><div><button :disabled="busy || !ready" @click="backup('import')">恢复备份</button><button class="outline" :disabled="busy || !ready" @click="backup('export')">↥ 导出备份</button></div></header>
      <div class="workspace">
