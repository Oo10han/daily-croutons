...
const datesWithEntries = computed(() => new Set(records.value.map(d => d.date)))
...
async function backup(action: 'export' | 'import') {
  ...
  try {
    ...
    if (action === 'export') { if (await window.journal.exportBackup()) notice.value = '备份已导出，请妥善保存。' }
      ...
      if (count !== null) { await refresh(); await load(selected.value); notice.value = `已恢复 ${count} 篇日记。` }
    }
  } catch (e) { report(e) } finally { busy.value = false }
}
...
onMounted(async () => {
  ...
  unsubscribe = window.journal?.onCloseRequest(async () => {
    ...
    busy.value = true
    try { await flush(); window.journal.closeReady() }
...
<template>
  <div class="shell">
    <aside class="sidebar">
      ...
      <div class="nav-current"><span>▤</span> 我的日记 <span class="nav-count">{{ records.length }}</span></div>
      <section class="calendar" aria-label="日历">
        ...
        <div class="calendar-foot"><span>本月记录了 {{ monthCount }} 天</span><button :disabled="busy || !ready" @click="select(today)">回到今天 ↗</button></div>
      </section>
      ...
      <div class="sidebar-bottom"><span class="local-dot"></span> 本地手账 · 无需联网 <small>v0.1</small></div>
    </aside>
    <main>
      <header class="topbar"><span>我的手账 <span class="slash">/</span> 日记</span><div><button :disabled="busy || !ready" @click="backup('import')">恢复备份</button><button class="outline" :disabled="busy || !ready" @click="backup('export')">↥ 导出备份</button></div></header>
      <div class="workspace">
        <div class="page-heading"><div><div class="eyebrow">{{ selectedDate.getFullYear() }} · 慢慢记录，好好生活</div><h1>{{ heading }} <span>{{ weekday }}</span></h1></div><span class="date-badge">{{ selected === today ? '今天' : '往日时光' }}</span></div>
        ...
        <article class="paper">
          ...
          <footer class="paper-footer"><span>{{ wordCount.toLocaleString() }} 字 <span class="footer-dot">·</span> 随写随存</span><div><button class="delete-button" :disabled="busy || !ready || (!datesWithEntries.has(selected) && !dirty)" @click="deleteOpen = true">删除日记</button><button class="save-button" :disabled="busy || !ready || saving || !dirty" @click="saveNow">保存日记</button></div></footer>
        </article>
        <div class="bottom-note"><span>✦ 平凡的一天，也值得被记住。</span><span>Ctrl / ⌘ + S 保存</span></div>
      </div>
    </main>
