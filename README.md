...
- 收支日历显示每天的收入和支出，点击日期查看当天全部明细。
- 按早餐、午餐、晚餐和加餐记录饮食，支持备注、编辑和删除。
- 按天记录体重，查看月度趋势和相对本月首次记录的变化。

v0.4.0 包含日记、记账、饮食和体重记录，暂不支持图片附件和多设备同步。截图使用示例数据，首次打开时记录为空。
...
进入账本默认显示「收支日历」，绿色「收」和棕色「支」分别表示当天收入与支出；无账目的日期显示横线，相邻月份的日期不显示金额。点击日期进入当天明细，也可切换到「收支明细」查看列表。日历始终汇总整月全部账目，不受列表筛选影响。

新增、修改、删除或恢复备份后，日历会同步更新。金额过长时悬停查看完整数值，侧栏日期悬停也能查看当天收支。此版本无需迁移数据库，继续使用 v3 备份格式。

![收支日历](preview-calendar.png)
